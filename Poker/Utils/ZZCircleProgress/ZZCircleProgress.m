...
-(void)setFinish_score:(double)finish_score
{
    if (_finish_score != finish_score) {
        _finish_score = finish_score;
        [self setNeedsDisplay];
    }
}
...
- (void)drawRect:(CGRect)rect {
    ...
    if (_showProgressText) {
        ...
        height=(CircleSelfHeight-stringSize.height)/3.0;
        CGRect r = CGRectMake(CircleSelfWidth/3.2-20,height*1.2,stringSize.width, stringSize.height);
        height=r.size.height+r.origin.y;
        
        
        NSMutableAttributedString *attr=[[NSMutableAttributedString alloc] initWithString:currentText];
        [attr addAttributes:attributes range:NSMakeRange(0, currentText.length-1)];
        [attr addAttributes:@{NSParagraphStyleAttributeName:textStyle,NSForegroundColorAttributeName:[UIColor whiteColor],NSFontAttributeName:[UIFont systemFontOfSize:18]} range:NSMakeRange(currentText.length-1, 1)];
...
//        [currentText drawInRect:r withAttributes:attributes];
    ...
    if (_finish>=0.0) {
        NSString *currentText = [NSString stringWithFormat:@"已完成:%.0f",_finish];
        //段落格式
        NSMutableParagraphStyle *textStyle = [[NSMutableParagraphStyle defaultParagraphStyle] mutableCopy];
        textStyle.lineBreakMode = NSLineBreakByWordWrapping;
        textStyle.alignment = NSTextAlignmentCenter;//水平居中
        //字体
        UIFont *font = [UIFont systemFontOfSize:0.05*CircleSelfWidth];
        //构建属性集合
        NSDictionary *attributes = @{NSFontAttributeName:font, NSParagraphStyleAttributeName:textStyle,NSForegroundColorAttributeName:[UIColor whiteColor]};
        //获得size
        CGSize stringSize = [currentText sizeWithAttributes:attributes];
...
//        float height=(CircleSelfHeight-stringSize.height)/3.0;
        CGRect r = CGRectMake(CircleSelfWidth/3.2,height,stringSize.width, stringSize.height);
    ...
    if (_total>=0.0) {
        NSString *currentText = [NSString stringWithFormat:@"总目标:%.0f",_total];
        //段落格式
        NSMutableParagraphStyle *textStyle = [[NSMutableParagraphStyle defaultParagraphStyle] mutableCopy];
        textStyle.lineBreakMode = NSLineBreakByWordWrapping;
        textStyle.alignment = NSTextAlignmentCenter;//水平居中
...
//        float height=(CircleSelfHeight-stringSize.height)/3.0;
    ...
    if (_finish_score>=0.0) {
        NSString *currentText = [NSString stringWithFormat:@"已奖励:%.0f",_finish_score];
        //段落格式
        NSMutableParagraphStyle *textStyle = [[NSMutableParagraphStyle defaultParagraphStyle] mutableCopy];
        textStyle.lineBreakMode = NSLineBreakByWordWrapping;
        textStyle.alignment = NSTextAlignmentCenter;//水平居中
