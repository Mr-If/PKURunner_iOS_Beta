...
@interface JKImagePickerController ()<JKAssetsGroupsViewDelegate,UICollectionViewDataSource,UICollectionViewDelegate,JKAssetsViewCellDelegate,JKPhotoBrowserDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,CutChooseHeaderDelegate,PhotoTweaksViewControllerDelegate>
...
- (void)setUpProperties
{
    ...
    _nextBtn.frame = CGRectMake(0, 0, 70, 30);
    
    _nextBtn.titleLabel.font = [UIFont systemFontOfSize:14];
    _nextBtn.layer.cornerRadius = 5;
    _nextBtn.layer.masksToBounds = YES;
    [_nextBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [_nextBtn addTarget:self action:@selector(finishPhotoDidSelected) forControlEvents:UIControlEventTouchUpInside];
    
    UIBarButtonItem *preItem = [[UIBarButtonItem alloc] initWithCustomView:_nextBtn];
    self.navigationItem.rightBarButtonItem = preItem;
    _nextBtn.hidden = YES;
//    
...
-(UIBarButtonItem *)rightBarBtn {
    if (!_rightBarBtn) {
        _rightBarBtn = [[UIBarButtonItem alloc]initWithTitle:@"" style:UIBarButtonItemStylePlain target:self action:@selector(finishPhotoDidSelected)];
    }
    return _rightBarBtn;
}
...
- (void)browerPhotoes:(NSArray *)array page:(NSInteger)page isYuLang:(BOOL)isYu;
{
        ...
        [self.assetsLibrary assetForURL:jka.assetPropertyURL
                            resultBlock:^(ALAsset *asset) {
                                ...
                                if (assets.count == weakSelf.selectedAssetArray.count) {
                                    [weakSelf browerPhotoes:assets page:0 isYuLang:YES];
                                }
...
- (void)loadAllAssetsForGroups
{
    ...
    [self.selectAssetsGroup enumerateAssetsWithOptions:NSEnumerationReverse usingBlock:^(ALAsset *result, NSUInteger idx, BOOL *stop) {
        if (result) {
            numberOfAssets++;
...
//        [self.navigationItem setLeftBarButtonItem:cancelItem animated:NO];
         ...
         self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:@"取消" style:UIBarButtonItemStylePlain target:self action:@selector(cancelEventDidTouched)];
    } else {
...
//        CutImageViewController *cutImageCon = [CutImageViewController new];
//        cutImageCon.chooseImage = avatarImage;
//        cutImageCon.delegate = self;
//        [self.navigationController pushViewController:cutImageCon animated:YES];
        
        PhotoTweaksViewController *photoTweaksViewController = [[PhotoTweaksViewController alloc] initWithImage:avatarImage];
 ...
 [self.navigationController pushViewController:photoTweaksViewController animated:YES];
      ...
      [self browerPhotoes:self.assetsArray page:[indexPath row]-1 isYuLang:NO];
    }
...
- (void)ChoosephotoBrowser:(JKPhotoBrowser *)photoBrowser didSelectAtIndex:(NSInteger)index {
...
- (void)ChoosephotoBrowser:(JKPhotoBrowser *)photoBrowser didDeselectAtIndex:(NSInteger)index {
    ALAsset *asset = photoBrowser.assetsArray[index];
    NSURL *assetURL = [asset valueForProperty:ALAssetPropertyAssetURL];
...
{
    self.nextBtn.hidden = (self.selectedAssetArray.count<=0);
    [self.nextBtn setTitle:[NSString stringWithFormat:@"下一步(%ld)",self.selectedAssetArray.count] forState:UIControlStateNormal];
...
//    self.finishLabel.centerY = self.finishButton.height/2;
    ...
    _showBigBtn.enabled = (self.selectedAssetArray.count>0);
...
//        [_toolbar addSubview:_selectButton];
        ...
        _showBigBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        _showBigBtn.frame = CGRectMake(0, 0, 60,30);
        [_showBigBtn setTitle:@"预览" forState:UIControlStateNormal];
        [_showBigBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [_showBigBtn setTitleColor:[UIColor lightGrayColor] forState:UIControlStateDisabled];
//        [_showBigBtn setBackgroundImage:[UIImage imageWithColor:[JKUtil getColor:@"f5f7fa"] size:_showBigBtn.size] forState:UIControlStateDisabled];
//      [_showBigBtn setBackgroundImage:[UIImage imageWithColor:color_themeColor size:_showBigBtn.size] forState:UIControlStateNormal];
        _showBigBtn.layer.cornerRadius = 5;
        _showBigBtn.layer.masksToBounds = YES;
        _showBigBtn.titleLabel.font = [UIFont systemFontOfSize:14];
        _showBigBtn.left = 20;
        _showBigBtn.centerY = _toolbar.height/2;
       
        [_showBigBtn addTarget:self action:@selector(previewPhotoesSelected) forControlEvents:UIControlEventTouchUpInside];
        _showBigBtn.enabled = NO;
        [_toolbar addSubview:_showBigBtn];
...
- (void)commpleteChooseCutImage:(UIImage *)headerImage andCutViewCon:(CutImageViewController *)cutCon {
    if ([self.delegate respondsToSelector:@selector(cutViewCommpelteChooseImage:viewCon:)]) {
        [self.delegate cutViewCommpelteChooseImage:headerImage viewCon:cutCon];
    }
}

-(void)finishWithCroppedImage:(UIImage *)croppedImage viewCon:(PhotoTweaksViewController *)cutCon
{
    if ([self.delegate respondsToSelector:@selector(cutViewCommpelteChooseImage:viewCon:)]) {
        [self.delegate cutViewCommpelteChooseImage:croppedImage viewCon:cutCon];
    }
}
