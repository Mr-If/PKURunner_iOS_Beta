    ...
    if (self.checkButton.selected) {
        if (_isYuLang) {
            if ([_delegate respondsToSelector:@selector(photoBrowser:didDeselectAtIndex:)]) {
                [_delegate ChoosephotoBrowser:self didDeselectAtIndex:self.currentPage];
            }
        }else {
            if ([_delegate respondsToSelector:@selector(photoBrowser:didDeselectAtIndex:)]) {
        ...
        if (_isYuLang) {
            if ([_delegate respondsToSelector:@selector(photoBrowser:didSelectAtIndex:)]) {
                [_delegate ChoosephotoBrowser:self didSelectAtIndex:self.currentPage];
            }
        }else {
            if ([_delegate respondsToSelector:@selector(photoBrowser:didSelectAtIndex:)]) {
...
- (UIView *)topView{
    if (!_topView) {
        ...
        UIImage  *imgH = [UIImage imageNamed:@"pick_button_blue"];
